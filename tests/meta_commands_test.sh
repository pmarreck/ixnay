...
set +e
loud_output="$(IXNAY_NO_COLOR=1 IXNAY_MUTE_CMD_ECHO=1 IXNAY_TEST_RUNNER="$stub_runner" "$ROOT_DIR/ixnay" test foo 2>&1)"
loud_status=$?
set -e
...
set +e
muted_output="$(IXNAY_NO_COLOR=1 IXNAY_MUTE_CMD_ECHO=1 IXNAY_TEST_RUNNER="$stub_runner" "$ROOT_DIR/ixnay" --test bar 2>&1)"
muted_status=$?
set -e
...
set +e
about_short="$(IXNAY_NO_COLOR=1 "$ROOT_DIR/ixnay" -a 2>&1)"
about_short_status=$?
set -e
...
set +e
about_long="$(IXNAY_NO_COLOR=1 "$ROOT_DIR/ixnay" --about 2>&1)"
about_long_status=$?
set -e
